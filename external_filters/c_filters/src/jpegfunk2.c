 ...
 */
void apply_filter(struct rgb_image *input_image, struct rgb_image *output_image, int numthreads) {
    int row, col, rgb;
    ...
    // Be careful about altering the time code, has a propensity to emit negative values
    // Here be dragons
    printf("Time for thread %d: %.2lf seconds\n", omp_get_thread_num(), ((1000000.0 * (time_end.tv_sec - time_start.tv_sec) + (1.0 * (time_end.tv_usec - time_start.tv_usec)))/1000000.0));

    // Print times to text filefor the TODO
    FILE *thread_times_file;
    thread_times_file = fopen("jpegfunk2_thread_times.txt", "a");
    if (thread_times_file == NULL) {
    	printf("Error creating thread times file!");
    	exit(1);
    }
    fprintf(thread_times_file, "Time for thread %d: %.2lf seconds\n", omp_get_thread_num(), ((1000000.0 * (time_end.tv_sec - time_start.tv_sec) + (1.0 * (time_end.tv_usec - time_start.tv_usec)))/1000000.0));
    fclose(thread_times_file);
    }
}
...
int main(int argc, char **argv) {
    ...
    if (argc != 4) {
        fprintf(stderr, "Usage: %s <input jpg file path> <output jpg file path> <# threads>\n", argv[0]);
        exit(1);
    }
